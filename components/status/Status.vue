...
<template>
	<div class="flex flex-col max-w-full gap-y-1 font-inter cursor-pointer">
		<div v-if="type !== PostType.Tiny"
			class="flex flex-row overflow-hidden text-[0.95rem] text-ellipsis whitespace-nowrap w-full">
			<NuxtLink :to="`/user/${status.account.id}`" class="flex-shrink-0 mr-2"
				:title="`${status.account.display_name}'s profile'`">
				<img loading="lazy" alt="" :src="status.account.avatar" :class="[
			...
			<div class="flex flex-row justify-between">
				<h4 class="font-bold dark:text-gray-200" :title="status.account.display_name" v-html="
				withEmojis(
					status.account.display_name,
					status.account.emojis
				)">
				</h4>
					<RouterLink :to="`/posts/${status.id}`"
		...
		<div class="flex flex-col gap-y-1 text-sm">
			...
			<p ref="textRef" v-html="withEmojis(status.content, status.emojis)" :class="
			classNames(
			...
			<StatusAttachments :type="type" :status="status" v-if="status.media_attachments.length > 0" />
		</div>
