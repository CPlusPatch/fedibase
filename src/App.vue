...
if (!store.auth.instance) store.client?.getInstance().then(res => {
...
watch(() => store.auth.token, () => {
	store.client = generator(store.auth.type as any, store.auth.url, store.auth.token);
})
</script>
	...
	<template v-if="store.client && store.auth.token" :key="store.auth.token">
		...
		<div class="relative duration-200 font-inter dark:bg-dark-800 flex h-screen w-screen">
			...
			<div class="flex overflow-hidden h-screen duration-200 grow flex-col relative">
				<div class="grid grid-cols-6 bg-gradient-light h-full md:grid-cols-11 rounded-t-lg">
					<div class="hidden h-full md:col-span-3 md:block overflow-hidden no-scroll">
						<LeftSidebar />
					</div>
					<div
