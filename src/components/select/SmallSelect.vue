...
<template>
	<Listbox :name="name" v-model="selected" as="div" class="relative font-inter" @update:modelValue="value => emit('update:modelValue', value)">
		<ListboxButton title="Open select menu"
			...
			<component :is="selected.icon" :strokeWidth="2" class="w-6 h-6" aria-hidden="true" />
		</ListboxButton>
		<ScaleFadeSlide>
			<ListboxOptions :class="[
				...
				<ListboxOption v-for="item in items" :key="item.value" :value="item" :class="[
					item.value === selected.value && 'bg-orange-100 dark:bg-orange-500/10',
					...
					<component :is="item.icon" class="w-5 h-auto text-gray-500" aria-hidden="true" />
