...
import { IconSun, IconMoon, IconHome, IconBell, IconPencilPlus } from "@tabler/icons-vue";
...
<template>
	<header
		...
		(store.state.postViewer || store.state.notifications || store.state.composer) && 'hidden']">
		...
		<Button theme="gray" title="Open notifications"
			@click="openNotifications"
			class="!p-3 !border-none !shadow-none !bg-white dark:!bg-transparent">
			<IconBell aria-hidden="true" />
		</Button>
		<Button theme="gray" title="Your avatar" class="!p-0 !border-none !shadow-none !bg-white dark:!bg-transparent">
			<img :src="store.auth.data?.avatar" class="w-9 h-9 rounded border-2" alt="Your avatar" />
		</Button>
	</header>

	<TransitionRoot as="template" :show="store.state.notifications &&
		_window.innerWidth < 768">
		<Dialog as="div" :unmount="false" class="flex fixed inset-0 max-w-full pointer-events-none">
				<TransitionChild :unmount="false" as="template" enter="ease-out duration-100"
					enterFrom="opacity-0 translate-y-4 translate-y-0 scale-95" enterTo="opacity-100 translate-y-0 scale-100"
					leave="ease-in duration-100" leaveFrom="opacity-100 translate-y-0 scale-100"
					leaveTo="opacity-0 translate-y-4 translate-y-0 scale-95">
					<DialogPanel class="overflow-hidden relative w-screen max-w-md pointer-events-auto">
						<div class="flex overflow-y-hidden flex-col pt-6 h-full bg-white shadow-xl dark:bg-dark-800">
							<div class="flex justify-between px-4 sm:px-6">
								<DialogTitle class="text-lg font-medium text-gray-900 dark:text-gray-50">
									Notifications
								</DialogTitle>
								<button type="button"
	...
	<TransitionRoot as="template" :show="store.state.postViewer &&
		_window.innerWidth < 768">
			<Dialog as="div" :unmount="false" class="flex fixed inset-0 max-w-full pointer-events-none">
					<TransitionChild :unmount="false" as="template" enter="ease-out duration-100"
						enterFrom="opacity-0 translate-y-4 translate-y-0 scale-95" enterTo="opacity-100 translate-y-0 scale-100"
						...
						<DialogPanel class="overflow-hidden relative w-screen max-w-md pointer-events-auto">
							<div class="flex overflow-y-hidden flex-col pt-6 h-full bg-white shadow-xl dark:bg-dark-800">
								...
								<div class="flex overflow-hidden relative mt-6 max-w-full grow sm:px-6">
									<Conversation :title="false" :id="store.viewingConversation" />
								</div>
							</div>
						</DialogPanel>
					</TransitionChild>
