...
import { IconList, IconListDetails, IconRocket, IconStarFilled } from '@tabler/icons-vue';
import SmallSelect, { SelectDirection, SelectItem } from '../select/SmallSelect.vue';
...
<template>
	<div class="flex flex-col gap-y-6 w-full max-w-full min-h-full font-inter">
		<div v-if="props.title" class="flex flex-row justify-between items-center">
			<h3 class="text-lg font-bold dark:text-gray-50">
				Notifications
			</h3>
			<SmallSelect
		...
		<ul class="flex overflow-y-scroll flex-col gap-y-3 max-w-full h-full no-scroll">
