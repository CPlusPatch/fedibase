...
<template>
	<div class="inline">
		<Link :href="`/posts/${status.id}`" :sidebar="status.id" class="text-xs text-gray-600 hover:underline dark:text-gray-300">
			<IconMessage class="inline mr-1 w-4 h-4" />
			Replying to <span v-if="replyStatus" v-html="withEmojis(replyStatus.account.display_name, replyStatus.account.emojis)"></span>
		</Link>
	</div>
</template>