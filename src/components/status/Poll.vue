...
<template>
	<form class="list-inside flex-col gap-y-2 flex" action="#" @submit="onSubmit">
		<li v-for="option, index of _poll.options" v-if="_poll"
			...
			<input v-if="!_poll?.voted" :type="_poll?.multiple ? 'checkbox' : 'radio'" name="poll"
		...
		<div class="text-sm text-gray-500 dark:text-gray-400">
			<Button theme="gray" type="submit" class="!px-2 !py-1 mr-2" v-if="!_poll?.voted">
				Vote
			</Button>

			{{ _poll?.votes_count }} people voted &middot; {{ _poll?.expired ? "Poll ended" : "Poll ends" }} {{ fromNow(new Date(_poll?.expires_at ?? "")) }}
		</div>
	</form>
</template>