...
<template>
	<div class="flex justify-between px-5 mt-3 w-full text-gray-700 dark:text-gray-400">
		<button @click="e => {
		...
		<button title="Add reaction" class="flex justify-center !static hover:animate-hithere">
			<IconMoodHappy aria-hidden="true" class="w-5 h-5" />
		</button>
		<button @click="e => {
			store.quotingTo = status;
			store.state.composer = true;
		}" title="Quote this post" class="flex justify-center !static hover:animate-hithere">
