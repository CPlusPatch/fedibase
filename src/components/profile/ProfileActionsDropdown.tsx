...
export const ProfileActionsDropdown = memo(
	({
		...
		const [relationship, setRelationship] =
			useState<Entity.Relationship>(initialRelationship);
						...
						<Menu.Item
							...
							onClick={(e: any) => {
								...
								if (relationship?.muting) {
									client
										?.unmuteAccount(user.id)
										.then(res => {
											setRelationship(res.data);
											toast.success(
												"Account posts unhidden"
											);
										})
										.catch(err => {
											console.error(err);
											toast.error(
												"Couldn't unhide account posts :/"
											);
										});
								} else {
									client
										?.muteAccount(user.id, false)
										.then(res => {
											setRelationship(res.data);
											toast.success(
												"Account posts will now be hidden"
											);
										})
										.catch(err => {
											console.error(err);
											toast.error(
												"Couldn't hide account posts :/"
											);
										});
								}
							}}
								...
								if (relationship?.muting_notifications) {
									client
										...
										.catch(err => {
											console.error(err);
											toast.error(
												"Couldn't unmute account :/"
											);
										});
								} else {
									client
										?.muteAccount(user.id, true)
										.then(res => {
											setRelationship(res.data);
											toast.success(
