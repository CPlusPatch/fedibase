	...
	<TransitionRoot :show="store.state.composer" as="template">
		<Dialog as="div" class="block relative z-40" @close="closeModal">
			...
			<div class="overflow-y-auto fixed inset-0 no-scroll py-5">
				<div class="flex justify-center items-start p-4 min-h-full text-center md:items-center sm:p-0">
					<TransitionChild as="template" enter="ease-in-out duration-200"
						...
						<DialogPanel class="relative my-8 w-full text-left transition-all transform sm:max-w-xl">
							<PostEditor />
						</DialogPanel>
					</TransitionChild>
				</div>
